...
To evaluate drawing games we simply run proof number search twice, the first time repeated positions are a loss for black and the second time for white.
...
\caption{DAG with corresponding tree.}
\label{treevsgraph}
\end{figure}
...
Though it has been shown that proof-number search can still perform quite good on directed acyclic graphs without any adaptaion,
on cyclic graphs it has to be adapted and can give incorrect results ~\cite{TODO}.\\
